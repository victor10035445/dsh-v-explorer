 ...
 * dsh-v-explorer — 宿主端。
 *
 * 为 Web 客户端的 V 文件探索器提供九个 API（全部限定在会话工作目录内）：
 *  - GET  /api/dsh-v-explorer/list?sessionId=&path=   列目录（懒加载一层）
 *  - GET  /api/dsh-v-explorer/file?sessionId=&path=   读文本文件（md/文本/代码预览）
 ...
 *    响应体携带更新后的列表）
 *  - GET  /api/dsh-v-explorer/events?sessionId=  SSE：fs.watch(recursive) 监听会话
 *    cwd，变更去抖后广播 {type:"fs-changed"}（不携带路径，客户端按需重拉展开目录；
 *    .dsh-v-explorer/ 子树事件被过滤——书签/快照写入不触发全树刷新信号）；书签增删另广播
 *    {type:"bookmarks-changed"}；连接归零即释放 watcher，插件卸载统一清理
 *
 ...
 * 逃出工作目录一律 403；文件读取有 10MB 上限与二进制检测。
 */

import { exec, execFile } from "node:child_process";
import { randomUUID } from "node:crypto";
import { watch as fsWatch } from "node:fs";
import { mkdir, readdir, readFile, stat, realpath, writeFile } from "node:fs/promises";
import { resolve as pathResolve, sep, dirname, basename, extname, isAbsolute } from "node:path";
import {
  parseRefTokens,
  buildExcerptPrompt,
...
/** 单文件读取上限：10MB。再大主要卡在浏览器端 JSON 解析与 markdown 渲染。 */
...
async function listDir(abs) {
  ...
  for (const d of dirents) {
    ...
    if (!isDir && d.isSymbolicLink()) {
      try {
        isDir = (await stat(abs + sep + d.name)).isDirectory();
      } catch {
        /* 断链符号链接按文件处理 */
      }
    }
    ...
    if (!isDir) {
      try {
        const s = await stat(abs + sep + d.name);
        size = s.size;
        mtimeMs = s.mtimeMs;
      } catch {
        /* 读不到 stat 的条目仍然列出 */
    ...
    entries.push({ name: d.name, isDir, size, mtimeMs, hidden: d.name.startsWith(".") });
  }
  entries.sort((a, b) => {
...
/* ------------------------------------------------------------------ *
 * 目录变更推送（SSE）：fs.watch(recursive) 监听会话 cwd，事件去抖后向
 * 该目录上的所有 EventSource 连接广播一条 {type:"fs-changed"} 信号
 * （不携带路径——客户端按需重拉展开目录；.dsh-v-explorer/ 子树事件被过滤，书签/
 * 快照写入不触发全树刷新信号）。最后一个连接关闭即释放 watcher；
 * 插件卸载时统一清理（见 apply 的 effect 返回值）。
 * ------------------------------------------------------------------ */

const WATCH_DEBOUNCE_MS = 500;
/** cwd realpath → 该目录上的 SSE 连接集合。 */
const sseConnections = new Map();
/** cwd realpath → { watcher, timer }（去抖计时器归属 watcher 条目）。 */
const dirWatchers = new Map();

/** 向挂在某 cwd 上的全部连接广播一次信号（type 参数化：fs-changed / bookmarks-changed）。 */
function broadcastChanged(cwdReal, type = "fs-changed") {
  const connections = sseConnections.get(cwdReal);
  if (!connections) return;
  const line = "data: " + JSON.stringify({ type }) + "\n\n";
...
/** 创建/复用一个目录 watcher；失败静默（客户端有 focus/可见性兜底刷新）。 */
function ensureDirWatcher(cwdReal) {
  if (dirWatchers.has(cwdReal)) return;
  let watcher;
  try {
    watcher = fsWatch(cwdReal, { recursive: true }, (eventType, filename) => scheduleBroadcast(cwdReal, filename));
  } catch {
    return; // 目录不可监听（权限/已删除）：不推送，不致命
  }
  watcher.on("error", () => {
    /* 目录被删/句柄失效：释放并广播一次（客户端重拉会看到最新状态）；
       仍有连接时短暂延迟后尝试重建（目录可能被恢复）。 */
    releaseDirWatcher(cwdReal);
    broadcastChanged(cwdReal);
    if (sseConnections.get(cwdReal)?.size) setTimeout(() => ensureDirWatcher(cwdReal), 2000).unref?.();
  });
  dirWatchers.set(cwdReal, { watcher, timer: null });
}

/** 事件去抖：一次保存/移动会触发一串事件，静止 500ms 后广播一次。
 *  .dsh-v-explorer/ 子树事件（书签/快照写入）不触发全树刷新信号——该目录对树不可见
 *  （隐藏过滤），过滤零副作用；filename 不可用的平台不过滤（宁多勿漏）。 */
function scheduleBroadcast(cwdReal, filename) {
  if (typeof filename === "string" && filename) {
    const normalized = filename.replace(/\\/g, "/");
    if (normalized === ".dsh-v-explorer" || normalized.startsWith(".dsh-v-explorer/")) return;
  }
  const entry = dirWatchers.get(cwdReal);
  if (!entry) return;
...
function releaseDirWatcher(cwdReal) {
  const entry = dirWatchers.get(cwdReal);
  if (!entry) return;
  if (entry.timer) clearTimeout(entry.timer);
  dirWatchers.delete(cwdReal);
  try {
    entry.watcher.close();
  } catch {
    /* 已关闭/句柄失效 */
...
/** 挂载一条 SSE 连接并确保其 cwd 有 watcher；返回连接清理函数。 */
function connectEvents(res, cwdReal) {
  ...
  connections.add(res);
  ensureDirWatcher(cwdReal);
  return () => {
    const set = sseConnections.get(cwdReal);
    if (!set) return;
    set.delete(res);
    if (set.size === 0) {
      sseConnections.delete(cwdReal);
      releaseDirWatcher(cwdReal);
    }
  };
}
...
export function apply(ctx) {
  ...
  ctx.effect(() => {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (route === "/list" && req.method === "GET") {
            ...
            const abs = await containedPath(cwd, url.searchParams.get("path") ?? "");
            const { entries, truncated } = await listDir(abs);
            json(res, 200, { cwd, path: abs, entries, truncated });
            return;
          }
          ...
          if (route === "/file" && req.method === "GET") {
            ...
            json(res, 200, { path: abs, name: basename(abs), ...result });
            return;
          }
          ...
          if (route === "/bookmark-add" && req.method === "POST") {
            ...
            if (result.added) {
              try {
                broadcastChanged(await realpath(cwd), "bookmarks-changed");
              } catch {
                /* 广播失败不影响响应 */
              }
            }
    ...
    return () => {
      dispose();
      /* 释放全部 watcher 并断开 SSE 连接（插件卸载/重载）。 */
      for (const cwdReal of [...dirWatchers.keys()]) releaseDirWatcher(cwdReal);
