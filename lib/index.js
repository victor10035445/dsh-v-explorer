...
async function persistedSessionCwd(ctx, sessionId) {
  ...
  try {
    /* rc.2 起持久化服务面改为按 id 观测：`stat(id)` 返回 SessionPersistenceSnapshot
     * （header + revision），旧的 `listSnapshots()` 已不存在（调用即 TypeError，
     * 0.1.5-rc.2 真机上兜底曾因此整体失效）。stat 精确对应"按 id 解析 cwd"的
     * 本兜底语义，且免去全量 list。 */
    const snapshot = await ctx.sessionPersistence?.stat?.(sessionId);
    const cwd = snapshot?.header?.cwd;
    if (typeof cwd === "string" && cwd) {
      persistedCwdCache.set(sessionId, cwd);
      return cwd;
    }
  } catch {
    /* 服务缺失/后端故障/会话不存在：回退为未找到 */
  }
  return null;
}
