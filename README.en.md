...
- **bookmarks tab** (new tab): session-scoped bookmark list, each bookmark a virtual subtree root (lazy loading, independent expansion); stale bookmarks kept with strikethrough; shares one repository with the files tree (★ badge + right-click toggle); persisted at `<cwd>/.dsh-v-explorer/bookmarks.json` (limit 100, idempotent dedup), multi-tab sync via SSE; bookmark subtrees follow the change stream (silent re-pull of expanded levels + immediate absent pruning, same cadence as the files tree)
- **Change-feed auto refresh**: a managed stream following the official ChangeFeed semantics, 500ms debounced silent re-pull of expanded directories (one shared orchestration across the files tree and the bookmarks tab); immediate absent pruning, visibility/focus fallbacks; directory reads handled by the official workspaceFiles Remote
- **Auto-expand** the official side panel on session entry with both files + bookmarks tabs pinned open (the two most-used pages, files kept in front): expands on every entry (new/switch/return/reload), state-driven with retries robust to mount timing; no cross-session "stay closed" memory — a manual collapse only affects the current session and is not fought while staying (low-frequency polling, only while installed); **default width** settles at the 300px contract minimum (the official first-open seed of 45% viewport is too wide — seeded through the official write path, in-session drag widths still respected)
